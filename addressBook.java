...
/*
        List<String> strDataBase = new ArrayList<String>(dataBase.size());
        for (Object object : dataBase) {
            strDataBase.add(Objects.toString(object, null));
        }
        String array[] = new String[strDataBase.size()];
        for(int j =0;j<strDataBase.size();j++){
            array[j] = strDataBase.get(j);
        }
        try {
            String path = "D:\\Codein Club Bootcamp\\assignments\\day16\\addressBookData.csv";
